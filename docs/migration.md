...
기존 Vercel의 `KAKAO_REST_API_KEY`는 `sensitive` 타입이라 Vercel API에서도 원문을 다시 읽을 수 없습니다. 새 원문을 확보해 Workbench Preview·Production에 설정하기 전까지 Kakao 장소 검색 API 검증은 보류합니다.
